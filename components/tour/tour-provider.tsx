...
import { TOURS, audienceMatches, type TourAudience, type TourStep } from "./registry/index";
import { pageTourIdFor } from "./route-match";
import { BRAND_OPTIONS, LOCALE, TOUR_THEME_CSS } from "./joyride-theme";
...
type TourContextValue = {
  ...
  hasPageTour: () => boolean;
  /** Whether the current audience has a welcome tour registered. */
  hasWelcome: () => boolean;
  /** Whether the current audience has a cross-page walkthrough registered. */
...
function isStepPresent(step: TourStep): boolean {
  // Sidebar anchors on the collapsed rail can be hidden — a viewport check has
  // to gate them, not just `querySelector`.
...
export function TourProvider({
  ...
  const welcomeKey = useMemo(() => `nomarc:tour:${audience}:welcome`, [audience]);

  // Keep the scroll offset clear of the sticky top bar, and track the
  // reduced-motion preference live (users flip it mid-session).
  useEffect(() => {
    const sync = () => {
      setScrollOffset(isDesktop() ? 96 : 76);
  ...
  // Joyride paints the overlay / spotlight / arrow with inline styles that read
  // CSS custom properties (see joyride-theme). Inject those properties once so
  // the chrome flips with Nomarc's `.dark` class strategy.
  useEffect(() => {
    if (typeof document === "undefined") return;
    const existing = document.getElementById("nm-tour-theme");
    if (existing) return;
    const style = document.createElement("style");
    style.id = "nm-tour-theme";
    style.textContent = TOUR_THEME_CSS;
  ...
  /** Find the welcome/walkthrough def that serves this audience (multi-audience aware). */
  const tourFor = useCallback(
    (kind: "welcome" | "walkthrough") => {
      const defs = Object.values(TOURS).filter((d) => d.kind === kind && audienceMatches(d.audience, audience));
      return defs[0] ?? null;
    },
    [audience],
  );
  ...
  const startWelcome = useCallback(() => {
    const def = tourFor("welcome");
    if (def) startTour(def.id);
  }, [tourFor, startTour]);

  const startWalkthrough = useCallback(() => {
    const def = tourFor("walkthrough");
    if (def) startTour(def.id);
  }, [tourFor, startTour]);
  ...
  const hasWelcome = useCallback(() => !!tourFor("welcome"), [tourFor]);
  const hasWalkthrough = useCallback(() => !!tourFor("walkthrough"), [tourFor]);
  ...
  // Auto-run the welcome tour once per audience — and RE-run whenever the
  // audience changes (the user unlocks or switches into a new role), so the tour
  // "takes over" for each intent the first time it's entered. Guarded by the
  // per-audience localStorage flag so it never nags twice, and skipped when no
  // welcome is authored for this audience yet.
  useEffect(() => {
    if (!tourFor("welcome")) return;
    let seen = false;
    try {
      if (localStorage.getItem(welcomeKey)) seen = true;
    } catch {
  ...
  }, [audience, welcomeKey, startWelcome, tourFor]);
