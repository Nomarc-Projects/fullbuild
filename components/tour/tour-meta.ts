import type { TourKind, TourStep } from "./registry/index";
 ...
 */
function groupSteps(steps: TourStep[]): {
  groupOf: number[];
  pageOf: (string | undefined)[];
  groupCount: number;
} {
...
export function withStepMeta(steps: TourStep[], kind: TourKind): TourStep[] {
  ...
  return steps.map((step, index) => {
    const stopsLeftOnPage = steps.filter(
      (_, j) => j > index && groupOf[j] === groupOf[index],
    ).length;
