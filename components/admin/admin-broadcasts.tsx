...
import {
  getAudienceCount, sendBroadcast, type AudienceFilter, type BroadcastLogEntry,
...
export function AdminBroadcasts({ history, mailConfigured }: { history: BroadcastLogEntry[]; mailConfigured: boolean }) {
  ...
  const filter: AudienceFilter = { role, plan, verifiedOnly };
  ...
  function send() {
    ...
    if (!window.confirm(`Send this email to ${count ?? "…"} recipient(s)? This can't be undone.`)) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-[#ececec] dark:border-white/10 bg-white dark:bg-[#1e1e1e] p-5 sm:p-6 space-y-4">
        <h2 className="text-[15px] font-bold text-[#1e1e1e] dark:text-white">Compose broadcast</h2>
        <Field label="Subject">
