...
class Media {
  extra = 0; geometry: any; gl: any; image: string; index: number; length: number; renderer: any; scene: any;
  screen: any; text: string; viewport: any; bend: number; textColor: string; borderRadius: number; font: string;
  program: any; plane: any; title: any; scale = 1; padding = 2; width = 0; widthTotal = 0; x = 0; speed = 0;
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        uniform mat4 modelViewMatrix; uniform mat4 projectionMatrix; uniform float uSpeed;
        varying vec2 vUv;
        void main(){
          vUv = uv;
          vec3 p = position;
          // Flat at rest; only ripples while scrolling (driven by uSpeed).
          p.z = (sin(p.x * 4.0) * 1.5 + cos(p.y * 2.0) * 1.5) * (uSpeed * 0.5);
          gl_Position = projectionMatrix * modelViewMatrix * vec4(p, 1.0);
        }`,
      fragment: `
        precision highp float;
        uniform vec2 uImageSizes; uniform vec2 uPlaneSizes; uniform sampler2D tMap; uniform float uBorderRadius;
        varying vec2 vUv;
...
class App {
  container: HTMLElement; scrollSpeed: number; scroll: any; renderer: any; gl: any; camera: any; scene: any;
  planeGeometry: any; medias: Media[] = []; mediasImages: GalleryItem[] = []; screen: any; viewport: any;
  isDown = false; start = 0; raf = 0; scroller: HTMLElement | Window;
  boundOnResize: any; boundOnTouchDown: any; boundOnTouchMove: any; boundOnTouchUp: any; boundOnScroll: any;
  lastScrollTop = 0;
  constructor(container: HTMLElement, { items, bend, textColor = "#ffffff", borderRadius = 0, font = "bold 28px Figtree", scrollSpeed = 2, scrollEase = 0.05, scroller }: any) {
  ...
  // Page scroll drives rotation like a wheel
  onScroll() {
    const st = this.scroller instanceof Window ? window.scrollY : (this.scroller as HTMLElement).scrollTop;
  ...
  addEventListeners() {
    ...
    (this.scroller as any).addEventListener("scroll", this.boundOnScroll, { passive: true });
  }
  destroy() {
    ...
    (this.scroller as any).removeEventListener("scroll", this.boundOnScroll);
    if (this.renderer?.gl?.canvas?.parentNode) this.renderer.gl.canvas.parentNode.removeChild(this.renderer.gl.canvas);
  }
}

export default function CircularGallery({
  items, bend = 2.5, textColor = "#ffffff", borderRadius = 0.05, font = "bold 28px Figtree", scrollSpeed = 2, scrollEase = 0.05,
}: {
  ...
  useEffect(() => {
    ...
    // Rotate with the nearest scrollable ancestor (dashboard <main>) or window.
    const scroller = (containerRef.current.closest(".overflow-y-auto") as HTMLElement) || window;
    const app = new App(containerRef.current, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, scroller });
    return () => app.destroy();
