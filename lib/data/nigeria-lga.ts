 ...
 *  Baked from the public nga-states-lga dataset so the profile form's
 *  State → LGA dropdowns need no runtime API call or key.
 *  37 states (incl. FCT) · 774 LGAs. */
export const NIGERIA_LGA: Record<string, string[]> = {
...
/** All state names, alphabetical — options for the State dropdown. */
export const NG_STATES = Object.keys(NIGERIA_LGA);

/** LGAs for a state (empty when the state is unknown/unset). */
export function lgasFor(state: string): string[] {
  return NIGERIA_LGA[state] ?? [];
}
